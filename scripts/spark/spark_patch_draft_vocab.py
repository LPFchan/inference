...
single-spark-vllm-tp1/patch/mtp_draft_vocab.py (Apache-2.0).

This is the Spark-side twin of docker/mangchi-vllm/patch_draft_vocab.py. The
Thor image is built with the patch baked in, but the Spark runs a stock public
image, so this variant reads and writes explicit paths and the caller bind
mounts the result. The compute_logits it rewrites is byte-identical in both
builds; only the module path and the class name for the helper anchor differ.

    spark_patch_draft_vocab.py <mtp.py in> [<mtp.py out>]
"""
...
mtp = Path(sys.argv[1])
out = Path(sys.argv[2]) if len(sys.argv) > 2 else mtp
